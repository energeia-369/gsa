<?php
$pageTitle = "GSA Carousel Management";
...
if (isset($_POST['delete_event_id'])) {
    ...
    $stmt = $pdo->prepare("DELETE FROM gsa_carousel_events WHERE id = ?");
    $stmt->execute([$delId]);
    echo "<script>window.location.href='admin-gsa-carousel.php?msg=deleted';</script>";
    exit();
}
...
if (isset($_POST['bulk_action']) && isset($_POST['selected_ids'])) {
    ...
    if (!empty($ids)) {
        ...
        if ($action === 'delete') {
            $stmt = $pdo->prepare("DELETE FROM gsa_carousel_events WHERE id IN ($placeholders)");
            $stmt->execute($ids);
        } elseif ($action === 'publish') {
            $stmt = $pdo->prepare("UPDATE gsa_carousel_events SET status = 'published' WHERE id IN ($placeholders)");
            $stmt->execute($ids);
        } elseif ($action === 'unpublish') {
            $stmt = $pdo->prepare("UPDATE gsa_carousel_events SET status = 'draft' WHERE id IN ($placeholders)");
            $stmt->execute($ids);
        }
        echo "<script>window.location.href='admin-gsa-carousel.php?msg=bulk_updated';</script>";
        exit();
    }
}
...
// Fetch events
$events = $pdo->query("SELECT * FROM gsa_carousel_events ORDER BY $orderBy")->fetchAll();
?>
...
<div class="admin-dashboard px-4 sm:px-8 py-10" style="background: #0b0c10; color: #f5f6fa; min-height: 100vh;">
  ...
  <div class="admin-header" style="border-bottom: 1px solid rgba(197, 168, 92, 0.2); padding-bottom: 20px;">
    <div class="header-left">
      ...
      <h1>GSA Carousel Events</h1>
      <p>Manage the sports tournaments and events appearing on the GSA carousel.</p>
    </div>
  </div>
  ...
  <div style="margin-top: 30px; margin-bottom: 60px;">
        ...
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px;">
            <div>
                <a href="admin-gsa-carousel-edit.php" class="btn-gold">+ Add New Tournament</a>
            </div>
                    ...
                    <?php if (empty($events)): ?>
                        <tr>
                            <td colspan="9" style="text-align: center; padding: 30px;">
                                No sports tournaments found.<br><br>
                                <a href="admin-gsa-carousel-edit.php" style="color: #c5a85c; text-decoration: underline;">Create one now</a>
                            </td>
                        </tr>
                    <?php else: ?>
                        ...
                        <tr>
                            ...
                            <td>
                                <strong><?= htmlspecialchars($evt['tournament_name']) ?></strong>
                            </td>
                            <td>
                                <?php if ($evt['sport_category']): ?>
                                    <span style="background: rgba(197, 168, 92, 0.2); color: #c5a85c; padding: 2px 8px; border-radius: 12px; font-size: 0.8em; border: 1px solid rgba(197,168,92,0.5);">
                                        <?= htmlspecialchars(strtoupper($evt['sport_category'])) ?>
                                    </span>
                                <?php endif; ?>
                            </td>
                            <td>
                                <?= htmlspecialchars($evt['venue']) ?><br>
                                <small style="color: #aaa;"><?= htmlspecialchars(implode(', ', array_filter([$evt['state'], $evt['country']]))) ?></small>
                            </td>
                            ...
                            <td>
                                <a href="admin-gsa-carousel-edit.php?id=<?= $evt['id'] ?>" class="btn-outline" style="margin-right: 5px;">Edit</a>
